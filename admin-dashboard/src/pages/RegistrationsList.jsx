...
export default function RegistrationsList() {
  ...
  useEffect(() => {
    ...
    axiosInstance
      .get(`/admin/registrations?page=${page}&limit=${ITEMS_PER_PAGE}&search=${search}`)
      .then(({ data }) => {
        if (!cancelled) {
          ...
          <tbody>
            {registrations.map((reg, idx) => (
              <tr key={reg.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                ...
                <td style={tdStyle}>{reg.eventName || '—'}</td>
                <td style={tdStyle}>{reg.date || '—'}</td>
                <td style={tdStyle}>{reg.status || '—'}</td>
                <td style={tdStyle}>
                  <button
